package org.codosoft.topis2.ui.screens.detail
...
import org.codosoft.topis2.common.LoadingIndicator
import org.codosoft.topis2.ui.screens.Screen
...
fun DetailScreen(vm: DetailViewModel, onBack: () -> Unit) {
    ...
    Screen {
        Scaffold(
            topBar = {
                DetailTopBar(title = state.movie?.title ?: "",
                    onBack = onBack,
...
private fun MovieDetail(
    ...
    Column(
        ...
        Text(text = buildAnnotatedString {
            property(name = stringResource(Res.string.original_language), movie.originalLanguage)
            property(name = stringResource(Res.string.original_title), movie.originalTitle)
            property(name = stringResource(Res.string.release_date), movie.releaseDate)
            property(name = stringResource(Res.string.popularity), movie.popularity.toString())
            property(name = stringResource(Res.string.vote_average), movie.voteAverage.toString(),
                end = true)
        },
            modifier = Modifier
                .fillMaxWidth()
